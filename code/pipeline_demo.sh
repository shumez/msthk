...
SSDIR=/Volumes/BA46/ws/msthk
# ROOTDIR=/Volumes/$DISK
# WSDIR=$ROOTDIR/ws
# SSDIR=$WSDIR/$PROJ

# preferences: directry
SRCDIR=$SSDIR/sourcedata
DCMDIR=$SRCDIR/dcm
DERDIR=$SSDIR/derivatives
...
export SUBJECTS_DIR=${SSDIR}/derivatives/freesurfer/${FS_VER}
...
echo -e "${timestamp}\tfreesurfer\tsetup done" > $MPDIR/$SID/msthk_pipeline_log.txt
echo -e -n "${SID}\t${timestamp}\t${MS_THK_PIPELINE_VER}\t" >> $SSDIR/msthk_pipeline_log.txt
...
# print log: copy disk
timestamp=`date +"%Y-%m-%d %T"`
echo -e "${timestamp}\tcopy disk\tdone" >> $MPDIR/$SID/msthk_pipeline_log.txt
echo -e -n  "${timestamp}\t\t${DCMDIR}/${SID}/${NRUN}\t" >> $SSDIR/msthk_pipeline_log.txt
...
timestamp=`date +"%Y-%m-%d %T"`
echo -e "${timestamp}\tdcm2niix\tdone" >> $MPDIR/$SID/msthk_pipeline_log.txt
echo -e -n  "${timestamp}\t${D2NVER}\t${D2NDIR}/${SID}/${NRUN}\t" >> $SSDIR/msthk_pipeline_log.txt

# copy to bids folder
cp ${D2NDIR}/${SID}/${NRUN}/t1_mprage_sag_*.json ${SSDIR}/${SID}/anat/${SID}-${NRUN}_T1w.json
cp ${D2NDIR}/${SID}/${NRUN}/t1_mprage_sag_*.nii.gz ${SSDIR}/${SID}/anat/${SID}-${NRUN}_T1w.nii.gz
...
# print log: dicom import
timestamp=`date +"%Y-%m-%d %T"`
echo -e "${timestamp}\tcopy nii\tdone" >> $MPDIR/$SID/msthk_pipeline_log.txt
echo -e -n  "${timestamp}\t\t${SSDIR}/${SID}/anat/\t" >> $SSDIR/msthk_pipeline_log.txt
...
# print log
timestamp=`date +"%Y-%m-%d %T"`
echo -e "${timestamp}\tfreesurfer\trecon-all done" >> $MPDIR/$SID/msthk_pipeline_log.txt
echo -e -n  "${timestamp}\t${FS_VER}\t${SUBJECTS_DIR}/${SID}\t" >> $SSDIR/msthk_pipeline_log.txt
...
# print log
timestamp=`date +"%Y-%m-%d %T"`
echo -e "${timestamp}\tfreesurfer\tsegmentBS done" >> $MPDIR/$SID/msthk_pipeline_log.txt
echo -e -n "${timestamp}\t${FS_VER}\t${SUBJECTS_DIR}/${SID}\t" >> $SSDIR/msthk_pipeline_log.txt
...
# print log
timestamp=`date +"%Y-%m-%d %T"`
echo -e "${timestamp}\tfreesurfer\tsegmentHA done" >> $MPDIR/$SID/msthk_pipeline_log.txt
echo -e -n  "${timestamp}\t${FS_VER}\t" >> $SSDIR/msthk_pipeline_log.txt

${FREESURFER_HOME}/bin/segmentThalamicNuclei.sh $SID $SUBJECTS_DIR

# print log
timestamp=`date +"%Y-%m-%d %T"`
echo -e "${timestamp}\tfreesurfer\tsegmentThalamicNuclei done" >> $MPDIR/$SID/msthk_pipeline_log.txt
